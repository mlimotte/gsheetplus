...
(defn with-retries
  ...
  [f err-message]
  (do-with-retries {:message err-message} f))
  ...
  (when (seq data-rows)
    (span/with-span! ["gsheetplus/append-rows!" {:spreadsheet.id spreadsheet-id
      ...
      (let [sheet-info' (sheet-info service spreadsheet-id sheet-id)
            ...
            row-count (get-in sheet-info' ["properties" "gridProperties" "rowCount"])
        ...
        (when (< row-count (+ start-row (count data-rows)))
          (let [num-rows (- (+ start-row (count data-rows)) row-count)]
            (log/info "inserting additional rows" {:start row-count :num-rows num-rows})
            (exec! service spreadsheet-id
                   [(insert-rows-request sheet-id true row-count num-rows)])))
